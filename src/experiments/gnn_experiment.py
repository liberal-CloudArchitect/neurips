...
def update_config_for_gnn(config: Dict) -> Dict:
    """更新配置以适配GNN模型"""
    
    # 添加GNN相关配置
    if 'gnn' not in config.get('models', {}):
        config['models']['gnn'] = {
            'hidden_dim': 256,
            'num_layers': 4,
            'dropout': 0.2,
...
def run_gnn_experiment(config_path: str = None, 
                      data_dir: str = "data",
                      model_type: str = "gat",
    ...
    config = update_config_for_gnn(config)
    ...
    logger.info("=" * 80)
    logger.info(f"开始第三阶段GNN实验 - {model_type.upper()}模型")
    logger.info("=" * 80)
    ...
    try:
        ...
        # 3. 初始化GNN训练器
        logger.info("步骤 3: 初始化GNN训练器")
        trainer = GNNTrainer(config)
        
        # 4. 训练GNN模型
        logger.info(f"步骤 4: 开始训练 {model_type.upper()} 模型")
        ...
        for task, (model, history) in trained_models.items():
            ...
            results_summary.append({
                ...
                'Num_Parameters': 'N/A'  # 将在后续版本中添加参数计数
            })
        ...
        # 创建结果目录
        results_dir = Path(config['output']['results_dir']) / 'gnn_models'
        results_dir.mkdir(parents=True, exist_ok=True)
        
        # 保存模型
        models_to_save = {task: model for task, (model, _) in trained_models.items()}
        trainer.save_models(models_to_save, str(results_dir))
        
        # 保存实验报告
        results_df = pd.DataFrame(results_summary)
        report_path = results_dir / f"gnn_{model_type}_experiment_report.csv"
        results_df.to_csv(report_path, index=False)
        logger.info(f"实验报告已保存: {report_path}")
        
        # 保存训练历史
        history_path = results_dir / f"gnn_{model_type}_training_history.pkl"
        import pickle
        ...
        if test_data_path.exists():
            ...
            submission_path = results_dir / f"gnn_{model_type}_submission.csv"
            submission_df.to_csv(submission_path, index=False)
            logger.info(f"提交文件已保存: {submission_path}")
        
        # 8. 总结
        logger.info("\n" + "=" * 80)
        logger.info("GNN实验完成总结")
        logger.info("=" * 80)
...
def compare_gnn_models(config_path: str = None, 
                      data_dir: str = "data",
    ...
    logger = setup_logger(name="gnn_comparison")
    logger.info("开始GNN模型对比实验")
    ...
    for model_type in models:
        ...
        try:
            trained_models, results = run_gnn_experiment(
    ...
    if len(all_results) > 1:
        ...
        # 保存对比结果
        results_dir = Path("results/gnn_models")
        results_dir.mkdir(parents=True, exist_ok=True)
        comparison_path = results_dir / "gnn_model_comparison.csv"
        comparison_df.to_csv(comparison_path, index=False)
        logger.info(f"\n对比结果已保存: {comparison_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GNN实验脚本")
    parser.add_argument("--config", type=str, default="configs/config.yaml", help="配置文件路径")
    parser.add_argument("--data_dir", type=str, default="data", help="数据目录")
    parser.add_argument("--model_type", type=str, default="gat", choices=['gat', 'mpnn'], help="模型类型")
